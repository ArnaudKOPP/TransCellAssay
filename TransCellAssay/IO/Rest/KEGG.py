...
where "db" is the database name or its abbreviation shown above and  "entry" is the entry name or the accession number
that is uniquely assigned within the database. In reality "db" may be omitted, for the entry name called the KEGG
object identifier (kid) is unique across KEGG.::
...
class KEGG(REST):
    ...
    **hsa** organism, type::
    ...
    the human organism::
        ...
        print(s.list("pathways", organism="hsa"))
    ...
    def _checkDB(self, database=None, mode=None):
        ...
        if mode == "info":
            if database not in KEGG._valid_databases_info and isOrg is False:
                raise ValueError("\033[0;31m[ERROR]\033[0m Database or organism provided is not correct (mode=info)")
        elif mode == "list":
            if database not in KEGG._valid_databases_list and isOrg is False:
                raise ValueError("\033[0;31m[ERROR]\033[0m Database is not correct (mode=list)")
        elif mode == "find":
            if database not in KEGG._valid_databases_find and isOrg is False:
                raise ValueError("\033[0;31m[ERROR]\033[0m Database is not correct (mode=find)")
        elif mode == "link":
            if database not in KEGG._valid_databases_link and isOrg is False:
                raise ValueError("\033[0;31m[ERROR]\033[0m Database is not correct (mode=link)")
        else:
